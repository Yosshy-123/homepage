...
import { USERNAME, TOKEN, MAX_PROJECTS, REQUEST_TIMEOUT_MS } from './config.mjs'
...
async function fetchPinnedRepos() {
  ...
  return nodes
    .filter((repo) => repo && !repo.isFork && isGitHubUrl(repo.url))
...
async function fetchTopOwnedRepos() {
  ...
  return repos
    .filter((repo) => !repo.fork && !repo.archived && isGitHubUrl(repo.html_url))
    .sort((a, b) => {
